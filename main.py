...
def main() -> None:
    ...
    print(f"パラメータ探索の組み合わせ数: {search_info['total_combinations']:,}")
    ...
    with yaspin(Spinners.line, text="Excelデータ抽出中") as spinner:
        try:
            time_array, value_array, excel_filename = DataExtractor.extract_from_directory(config.INPUT_DIR)
    ...
    with yaspin(Spinners.line, text="将来予測をプロット中") as spinner:
        try:
            ...
            output_excel_path = predictor.save_prediction_to_excel(
                forecast_time_array, forecast_value_array, excel_filename
            )
        except Exception as e:
            spinner.fail("💥 ")
    ...
    if output_excel_path:
        print(f"将来予測Excel: {output_excel_path}")
