...
def render_sidebar() -> Dict[str, Any]:
    ...
    k_min = st.sidebar.number_input("最小値 (K_min)", value=20.0, min_value=0.0, step=1.0, format="%.2f")
    k_max = st.sidebar.number_input("最大値 (K_max)", value=30.0, min_value=0.0, step=1.0, format="%.2f")
    k_step = st.sidebar.number_input("ステップ (K_step)", value=5.0, min_value=0.0, step=1.0, format="%.2f")
    ...
    gamma_min = st.sidebar.number_input("最小値 (γ_min)", value=0.0285, min_value=0.0, step=0.0001, format="%.4f")
    gamma_max = st.sidebar.number_input("最大値 (γ_max)", value=0.0350, min_value=0.0, step=0.0001, format="%.4f")
    gamma_step = st.sidebar.number_input("ステップ (γ_step)", value=0.0005, min_value=0.0, step=0.0001, format="%.4f")
    ...
    start_year = st.sidebar.number_input("データ開始年", value=1950, min_value=0, step=1)
    forecast_end_t = st.sidebar.number_input("予測年数 (データ開始年からの年数)", value=150, min_value=1, step=10)
