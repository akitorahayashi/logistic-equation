...
class LogisticEquationModel:
    ...
    def solve_runge_kutta(
        self, v0: float, t_start: float, t_end: float, dt: float = 0.1
        ...
        with np.errstate(over="raise", divide="raise", invalid="raise"):
            for i in range(1, n_steps):
                try:
                    ...
                    k2 = dt * self.differential_equation(
                        t[i - 1] + dt / 2, vs[i - 1] + k1 / 2
                    )
                    k3 = dt * self.differential_equation(
                        t[i - 1] + dt / 2, vs[i - 1] + k2 / 2
                    )
