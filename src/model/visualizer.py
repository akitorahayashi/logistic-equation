...
class FittingVisualizer:
    ...
    def plot_with_equation(
        ...
        ax.plot(display_time_model, scaled_value_model, '-',
...
class ForecastVisualizer:
    ...
    def plot_forecast(
        ...
        ax.plot(actual_display_time, scaled_value_array, 'o',
                label=f'実績データ ({actual_display_time[0]:.0f}-{actual_display_time[-1]:.0f})', 
                markersize=8, zorder=10)

        ax.plot(forecast_display_time, scaled_forecast_value_array, '-',
        ...
        ax.axvline(x=actual_display_time[-1], color='gray', linestyle='--',
